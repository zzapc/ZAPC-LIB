CLASS zcl_ap_gos DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF t_attachment,
           ...
           END OF t_attachment.
    TYPES tt_attachment TYPE STANDARD TABLE OF t_attachment.
    ...
    CLASS-METHODS get_file_list
      ...
      CHANGING  i_attachments   TYPE zt_sood             OPTIONAL
                ...
                lt_attachment   TYPE tt_attachment       OPTIONAL.
...
* http://scn.sap.com/docs/DOC-41922
    ...
    DATA: r_instid_b      TYPE RANGE OF srgbtbrel-instid_b,
          ...
          ls_key          TYPE ts_key,
          ls_attachment   TYPE t_attachment,
        ...
        LOOP AT ta_srgbtbrel INTO wa_srgbtbrel.
          ...
          TRY.
              ...
              LOOP AT lt_links INTO ls_link WHERE instid_b IN r_instid_b.
                ...
                ENDCASE.
              ENDLOOP.
            CATCH cx_obl_parameter_error. "#EC *
            CATCH cx_obl_internal_error. "#EC *
            CATCH cx_obl_model_error. "#EC *
            CATCH cx_root. "#EC *
          ENDTRY.
        ENDLOOP.
...
* GOS toolbar
    CREATE OBJECT l_manager
      ...
      EXCEPTIONS
        OTHERS       = 1.
    IF sy-subrc <> 0.
      MESSAGE 'Error creando toolbar'(ect) TYPE 'S'.
      RETURN.
    ENDIF.
      ...
      IF ar_object IS INITIAL.
        get_file_list( EXPORTING instid        = clave
                       ...
                       CHANGING  i_attachments = i_attachments
                                 lt_attachment = lt_attachment ).
        READ TABLE i_attachments INTO l_sood INDEX 1.       "#EC CI_NOORDER
        IF sy-subrc = 0.
          MOVE-CORRESPONDING l_sood TO l_sood4.
          READ TABLE lt_attachment INTO DATA(l_attach) INDEX 1.
          IF sy-subrc = 0.
            MOVE-CORRESPONDING l_attach TO l_sood4.
          ENDIF.
