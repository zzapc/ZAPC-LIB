...
CONSTANTS tabla TYPE tabname VALUE 'ZPARAMETROSF'.
...
SELECT-OPTIONS: s_clave FOR zparametrosf-clave.
SELECTION-SCREEN SKIP.

PARAMETERS: p_fecha TYPE dats DEFAULT sy-datum.
SELECTION-SCREEN SKIP.
...
CLASS zcl_report IMPLEMENTATION.
  METHOD buscar_datos.
    ...
    CLEAR i_listado.
    SELECT * FROM (tabla)
      INTO CORRESPONDING FIELDS OF TABLE i_listado_tot
     WHERE clave IN s_clave
      ORDER BY PRIMARY KEY.

    o_prog->o_sgpi->get_filas_tabla( i_listado[] ).
    LOOP AT i_listado_tot ASSIGNING FIELD-SYMBOL(<listado>).
      tabix = sy-tabix.
      <listado>-tabix = tabix.
      sgpi_texto( texto1 = 'Procesando datos'(pda) cant_porc = 100 ).

      validaciones( CHANGING listado = <listado> ).
      IF p_fecha IS INITIAL OR <listado>-begda <= p_fecha AND <listado>-endda >= p_fecha.
  ...
  METHOD status_dynpro_0100.
    ...
    o_alv->set_field_quitar( 'CHECK,MANDT,MESSAGE,TABIX,UPDKZ,CLAVE_INTERNA,CLAVE_INT_SF' ).

    IF p_vis IS INITIAL.
      o_alv->add_button( button = 'F01' text = 'Grabar' icon = icon_system_save ucomm = 'GRABAR' ).
      o_alv->add_button( button = 'F02' text = 'Delimitar' icon = icon_time ucomm = 'DELIMITAR' ).
      o_alv->add_button( button = 'F03' text = 'Desglosar' icon = icon_list ucomm = 'DESGLOSAR' ).

      IF sy-sysid = zcl_c=>entorno_desarrollo.
        o_alv->add_button( button = 'F04' text = 'Crear OT'  icon =  icon_import_transport_request ucomm = 'OT' ).
        o_alv->add_button( button = 'F05' text = 'Comparar con producción'  icon =  icon_compare ucomm = 'COMP' ).
      ELSE.
        o_alv->add_button( button = 'F05' text = 'Comparar con desarrollo'  icon =  icon_compare ucomm = 'COMP' ).
      ENDIF.
      o_alv->set_field_input( campos_input ).
    ENDIF.
    o_alv->set_field( campo = 'ENDDA' op = 'COL_AFTER' valor = 'BEGDA' ).
  ...
  METHOD command_dynpro_0100.
    DATA: l_comm_sel TYPE string VALUE 'OT,LIST_EMP,SIMULAR,DELIMITAR,DESGLOSAR',
          l_hay_sel  TYPE c LENGTH 1,
          l_cont     TYPE i.
    ...
    CASE ucomm.
      WHEN 'GRABAR'.
        LOOP AT i_listado ASSIGNING FIELD-SYMBOL(<listado>) WHERE updkz NE 'D'.
        ...
        CLEAR i_list_dup.
        LOOP AT i_listado ASSIGNING <listado> WHERE updkz NE 'D'.
          DATA(l_solapamiento) = ''.
          LOOP AT i_list_dup ASSIGNING FIELD-SYMBOL(<dup>) WHERE clave_int_sf = <listado>-clave_int_sf
                                                             AND clave_interna NE <listado>-clave_interna.

            IF zcl_ap_fechas=>hay_solapamiento_fechas( fini1 = <listado>-begda
                                          ffin1 = <listado>-endda
                                          fini2 = <dup>-begda
                                          ffin2 = <dup>-endda ).
              <dup>-tabix = 2.
            ...
            ENDIF.
          ENDLOOP.
          IF l_solapamiento IS INITIAL.
            l_list_dup-clave_interna = <listado>-clave_interna.
            l_list_dup-clave_int_sf = <listado>-clave_int_sf.
            l_list_dup-begda = <listado>-begda.
            l_list_dup-endda = <listado>-endda.
            l_list_dup-tabix = 1.
            APPEND l_list_dup TO i_list_dup.
          ENDIF.
          CLEAR <listado>-color.
        ENDLOOP.
        LOOP AT i_list_dup ASSIGNING <list> WHERE tabix > 1.
          LOOP AT i_listado ASSIGNING <listado> WHERE clave_int_sf = <list>-clave_int_sf.
            set_status_list( EXPORTING color = 'R' CHANGING list = <listado> ).
          ENDLOOP.
        ENDLOOP.
        IF sy-subrc = 0.
          MESSAGE 'No puede grabar si hay solapamientos' TYPE 'I'.
          o_alv->refrescar_grid( ).
          RETURN.
        ENDIF.
        ...
        LOOP AT i_listado ASSIGNING <listado> WHERE updkz <> ''.
          ...
          MOVE-CORRESPONDING <listado> TO zparametrosf.
          l_cont = l_cont + 1.
          IF <listado>-updkz = 'D'.
            ...
            IF sy-subrc = 0.
              DELETE i_listado_tot WHERE tabix = <listado>-tabix.
              DELETE i_listado.
            ENDIF.
          ELSE.
            CLEAR zparametrosf.
            MOVE-CORRESPONDING <listado> TO zparametrosf.
            MODIFY zparametrosf FROM zparametrosf.
            IF sy-subrc = 0.
              CLEAR: <listado>-updkz, <listado>-lights, <listado>-message, <listado>-style.
              i_listado_tot[ tabix = <listado>-tabix ] = <listado>.
            ENDIF.
          ENDIF.
        ENDLOOP.
      ...
      WHEN 'DELIMITAR'.
        DATA l_return.
        fecha = sy-datum.
        zcl_ap_popup=>popup_usuario( EXPORTING campo1 = 'T510-BEGDA'
                                               titulo = 'Delimitar validez: Entrada'
                                     IMPORTING return = l_return
                                     CHANGING  valor1 = fecha ).
        IF l_return NE 'A' AND NOT fecha IS INITIAL.
          LOOP AT i_listado ASSIGNING <listado> WHERE check = 'X' AND endda < fecha.
            MESSAGE 'No es posible delimitar todos los registros' TYPE 'I'.
            RETURN.
          ENDLOOP.
          LOOP AT i_listado ASSIGNING <listado> WHERE check = 'X' AND begda < fecha.
            CLEAR <listado>-check.
            l_listado = <listado>.
            <listado>-begda = fecha.
            validaciones( EXPORTING mod = 'U' CHANGING listado = <listado> ).
            l_listado-endda = fecha - 1.
            ADD 1 TO tabix.
            l_listado-tabix = tabix.
      ...
      WHEN 'DESGLOSAR'.
        LOOP AT i_listado ASSIGNING <listado> WHERE check = 'X'.
          LOOP AT i_listado_tot ASSIGNING FIELD-SYMBOL(<listado_tot>) WHERE clave_int_sf = <listado>-clave_int_sf.
            IF NOT line_exists( i_listado[ endda = <listado_tot>-endda ] ).
              APPEND <listado_tot> TO i_listado.
              APPEND <listado_tot> TO i_listado_ini.
            ENDIF.
          ENDLOOP.
        ENDLOOP.
        SORT i_listado BY clave.
        o_alv->refrescar_grid( soft_refresh = '' ).

      WHEN 'OT'.
        LOOP AT i_listado ASSIGNING <listado> WHERE check = 'X'.
          MOVE-CORRESPONDING <listado> TO zparametrosf.
          zparametrosf-mandt = sy-mandt.
          l_key = zparametrosf(longitud_clave).
  ...
  METHOD validaciones.
    ...
    IF NOT listado-updkz IS INITIAL.
      ...
      IF listado-message IS INITIAL.
        IF listado-endda < listado-begda.
          listado-message = 'Fecha fin no puede ser inferior a fecha inicio'.
          l_icono = icon_red_light.
          __add_lista campos_error 'BEGDA,ENDDA'.
        ENDIF.
      ENDIF.
    ENDIF.

    CLEAR: listado-clave_interna, listado-clave_int_sf.
    LOOP AT i_campos_tabla ASSIGNING <campo> WHERE keyflag = 'X'.
      ASSIGN COMPONENT <campo>-fieldname OF STRUCTURE listado TO <fs>.
      IF sy-subrc = 0.
        IF NOT ( <campo>-fieldname = 'BEGDA' OR <campo>-fieldname = 'ENDDA' ).
          CONCATENATE listado-clave_int_sf <fs> INTO listado-clave_int_sf SEPARATED BY '-'.
        ENDIF.
    ...
    IF listado-message IS INITIAL.
      DATA(l_solapamiento) = ''.
      LOOP AT i_listado_tot ASSIGNING FIELD-SYMBOL(<tot>) WHERE clave_int_sf = listado-clave_int_sf
                                                          AND clave_interna NE listado-clave_interna.
      ...
      ENDLOOP.
      IF l_solapamiento = 'X'.
        listado-message = 'Hay solapamiento con otro registro'.
        l_icono = icon_red_light.
        __add_lista campos_error 'BEGDA,ENDDA'.
      ENDIF.
    ENDIF.
    ...
    ASSIGN i_listado_tot[ tabix = listado-tabix ] TO <tot>.
    IF sy-subrc = 0.
      <tot> = listado.
    ELSE.
      APPEND listado TO i_listado_tot.
    ENDIF.
...
*    campos_obligatorios = campos_clave.
    campos_obligatorios = 'CLAVE,CAMPO,BEGDA,ENDDA'.
  ENDMETHOD.
