...
*        o_alv->refrescar_grid( ).
  ...
  METHOD status_dynpro_0100.
    ...
    IF o_alv_t IS INITIAL.
      ...
      o_alv_t->set_field_text( campo = 'TEXTO' valor = 'Transacción'(TRA) ).
...
*          CALL TRANSACTION zap_dev_est-objeto.
          ...
          ELSE.
            MESSAGE i398(00) WITH 'No existe transacción'(NET) zap_dev_est-objeto '' ''.
          ENDIF.
        WHEN 'PC'.
...
INITIALIZATION.
  ...
  IF sy-batch IS INITIAL.
    ...
    o_prog->o_alv->add_button( button = 'F07' text = 'Desarrollos'() icon = icon_calculation qinfo = 'Gestión desarrollos'(GD1) ucomm = 'DEV' ).
    ...
    o_prog->o_alv->add_button( button = 'M01' text = 'Descarga librería'(DL1) qinfo = 'Descarga librería' ).
    ...
    o_prog->o_alv->add_button( button = 'M04' text = 'Añadir OT'(AO1) qinfo = 'Añadir objetos de OT'(AOD) ).
