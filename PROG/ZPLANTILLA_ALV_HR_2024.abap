...
*    o_alv->add_button( button = 'F01' text = 'Excel'  icon = icon_xls ).
    ...
    o_alv->set_field_hotspot( campo = 'PERNR' auto = 'X' ).
...
* A veces hay dos empleados para la misma persona
  DELETE all_0000 WHERE NOT (     stat2 IN pnpstat2
                              AND (     begda <= pn-endda
                                    AND endda >= pn-begda ) ).
  SORT all_0000 BY pernr ASCENDING endda DESCENDING.
  DELETE ADJACENT DUPLICATES FROM all_0000 COMPARING pernr.
